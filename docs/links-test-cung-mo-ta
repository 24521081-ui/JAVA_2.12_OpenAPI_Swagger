...
- Người dùng có thể xem danh sách các endpoint, tham số đầu vào, dữ liệu mẫu và thử trực tiếp từng API bằng nút **Try it out**.
...
- Đây là phần rất quan trọng của đề tài vì nó thể hiện đúng ý tưởng **OpenAPI tự động sinh tài liệu API từ mã nguồn Java**, thay vì nhóm phải viết tài liệu API thủ công.

**Giải thích rõ hơn về “tự động sinh JSON”:**
- Nhóm chỉ cần viết code Java REST API.
...
- Người dùng có thể vừa xem tài liệu API, vừa kiểm thử trực tiếp trên giao diện Swagger UI
